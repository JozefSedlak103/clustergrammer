export default (function calc_spillover_triangles(store) {
  const viz_dim = store.select("visualization").viz_dim;
  ...
  const dendro_trap =
    store.select("dendro").trap_height + store.select("dendro").trap_float;
